...
> ⚠️ **Why appearance has a hard "never copy old value" rule.** Without this rule, the model copies the previous appearance verbatim every heartbeat. Seemingly reasonable rules made it worse: "it's normal not to change clothes all day" gave the model a license to never change; "check diary for recent outfit changes" failed because diary entries have no timestamps and the model couldn't find them. The fix is absolute: generate a new description every heartbeat, period. The sole exception is when grep detects an ongoing intimate/sexual scene — freezing appearance during sex is the only correct behavior. Everything else gets rewritten, forcing continuous variation.
