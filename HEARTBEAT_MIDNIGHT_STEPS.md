...
⚠️ **This file is for the 00:00 heartbeat only.** Daytime heartbeats use HEARTBEAT.md.

## Frequency

- **Night wrap-up (00:00):** One final pass after daily reflection to sync the last conversation + cross-day diary attribution + digest overwrite
...
**0b. Determine Diary Attribution Date**

Extract UTC timestamps from the grep #3 results and convert to UTC+8. The resulting date is the diary attribution date.

⚠️ If grep #3 has no output (no conversations), use `exec date -d 'yesterday' +%Y-%m-%d` to get the attribution date.
...
3. Overwrite the digest file using exec (echo > to initialize, then cat >> to append content):
   ...
   - Replace <attribution-date> with the date string from step 0b (e.g., 2026-04-27)

**Example:**
```
Afternoon — he said he was tired from a long day, just wanted to stay in.
Evening — talked about work stress, he seemed a bit down about a family matter.
Overall mood — tired but okay. Said goodnight before sleep.
```
...
After writing appearance, also set `context_note` — a brief note about anything unusual or noteworthy in the current situation (e.g., "He's been quiet since an argument", "Rainy day, she stayed in"). Leave empty if nothing notable.
